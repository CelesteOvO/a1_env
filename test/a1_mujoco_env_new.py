...
def quat_rotate_inverse(q, v):
    """
    Rotate a vector from global coordinates to local (base) coordinates
    using the inverse of a single quaternion.

    Args:
        q (np.ndarray): Quaternion [x, y, z, w], shape (4,).
        v (np.ndarray): 3D vector to be rotated, shape (3,).

    Returns:
        np.ndarray: Rotated vector in the local (base) coordinate frame.
    """
    # Ensure inputs are numpy arrays
    q = np.asarray(q)
    v = np.asarray(v)

    # Split quaternion into scalar (w) and vector (x, y, z) components
...
class A1MujocoEnv(MujocoEnv):
    ...
    ### TODO: quat rotate inverse
    def _quat_rotate_inverse(self, quat, vec):
        """
        Rotate a vector from global coordinates to local (base) coordinates
        using the inverse of the quaternion.

        Args:
            quat (np.array): Quaternion [w, x, y, z].
            vec (np.array): 3D vector to be rotated.

        Returns:
            np.array: Rotated vector in the local (base) coordinate frame.
        """
        w, x, y, z = quat
        quat_inv = np.array([w, -x, -y, -z])  # Quaternion inverse
        vec_quat = np.array([0.0, *vec])  # Convert vector to quaternion (0 + vx*i + vy*j + vz*k)

        # Quaternion multiplication: q_inv * vec_quat * q
        rotated_quat = self._quat_multiply(
            self._quat_multiply(quat_inv, vec_quat), quat
        )

        return rotated_quat[1:]  # Return the vector part (x, y, z)
    ...
    def _quat_multiply(self, q1, q2):
        ...
        Args:
            q1, q2 (np.array): Quaternions [w, x, y, z].

        Returns:
            np.array: Result of quaternion multiplication.
        """
        w1, x1, y1, z1 = q1
        w2, x2, y2, z2 = q2
    ...
    def _get_obs(self):
        ...
        global_linear_velocity = velocity[:3]
        global_angular_velocity = velocity[3:6]
        dofs_velocity = velocity[6:]

        # ### TODO: Convert global velocities to base (local) coordinate frame
        # w, x, y, z = self.data.qpos[3:7]  # Quaternion representing orientation
        # quat = np.array([w, x, y, z])
        # base_linear_velocity = self._quat_rotate_inverse(quat, global_linear_velocity)
        # base_angular_velocity = self._quat_rotate_inverse(quat, global_angular_velocity)
        ...
        curr_obs = np.concatenate(
            (
                global_linear_velocity * self._obs_scale["linear_velocity"],
                global_angular_velocity * self._obs_scale["angular_velocity"],
                projected_gravity,
    ...
    # def projected_gravity(self):
    #     # 获取机器人姿态的四元数
    #     w, x, y, z = self.data.qpos[3:7]  # 四元数 (w, x, y, z)
    #     quat = np.array([w, x, y, z])  # 转为数组形式
    #
    #     # 使用 quat_rotate_inverse 计算基座坐标系下的重力向量
    #     projected_gravity = quat_rotate_inverse(quat, self._gravity_vector)
    #
    #     # 归一化重力向量
    #     norm = np.linalg.norm(projected_gravity)
    #     if norm == 0:
    #         return projected_gravity  # 如果零向量直接返回
    #     else:
    #         return projected_gravity / norm  # 返回单位向量
    ...
    ### TODO: stumble Penalty, stand Still
    @property
    ...
    @property  # TODO: Not used, set self._max_contact_force = 100.0
    ...
    def _calc_reward(self, action):
        ...
        # print(f"ctrl_cost: {ctrl_cost}, action_rate_cost: {action_rate_cost}, vertical_vel_cost: {vertical_vel_cost}, xy_angular_vel_cost: {xy_angular_vel_cost}, joint_limit_cost: {joint_limit_cost}, joint_velocity_cost: {joint_velocity_cost}, joint_acceleration_cost: {joint_acceleration_cost}, orientation_cost: {orientation_cost}, collision_cost: {collision_cost}, default_joint_position_cost: {default_joint_position_cost}, height_cost: {height_cost}, termination_cost: {termination_cost}, stumble_cost: {stumble_cost}, stand_still_cost: {stand_still_cost}")
